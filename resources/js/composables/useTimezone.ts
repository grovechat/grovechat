...
export function useTimezone() {
    ...
    function getTimezones(lang: 'zh-CN' | 'en' = 'zh-CN') {
        return timezoneData.map(tz => ({
            value: tz.value,
            label: lang === 'zh-CN' ? `${tz.zh} ${tz.offset}` : `${tz.en} ${tz.offset}`,
            offset: tz.offset,
        }));
    }
    ...
    function getCurrentTimezoneInfo(lang: 'zh-CN' | 'en' = 'zh-CN') {
        const data = timezoneData.find(tz => tz.value === timezone.value) || timezoneData[0];
        return {
            value: data.value,
            label: lang === 'zh-CN' ? `${data.zh} ${data.offset}` : `${data.en} ${data.offset}`,
            offset: data.offset,
        };
    }

    // 格式化时间到当前时区
    function formatToTimezone(date: Date | string, format?: Intl.DateTimeFormatOptions): string {
        ...
        return new Intl.DateTimeFormat('zh-CN', format || defaultFormat).format(dateObj);
    }
